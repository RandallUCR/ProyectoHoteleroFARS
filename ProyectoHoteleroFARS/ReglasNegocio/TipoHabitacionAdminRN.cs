...
namespace ReglasNegocio
{
    public class TipoHabitacionAdminRN
    {
        ...
        public int modificarTiposHabitaciones(int id, string nombre, string desc, int precio, string base64, string formato)
        {
            TipoHabitacionAdminAD tad = new TipoHabitacionAdminAD();
            int result = -3;
            try
            {
                result = tad.modificarTiposHabitaciones(new TipoHabitacion()
                {
                    TN_Id = id,
                    TC_Nombre = nombre,
                    TC_Descripcion = desc,
